...
async fn map(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let resp = client.get("https://apexlegendsstatus.com/current-map").send().await?.text().await?;

    // Questionable but it works for now
    let pubs_map_reg = Regex::new("<h1 style=\"font-weight: 600; margin-bottom: 5px;\">Battle Royale: (.*?)</h1>").unwrap();
    let next_pubs_map_reg = Regex::new("<h5 style=\"margin-top: 0px;\">Next map is <b>(.*?)</b>, from (.*?) to (.*?) UTC</h5>").unwrap();
