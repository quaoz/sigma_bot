...
async fn synonyms(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    for entry in entries {
        let synonyms_list = entry.get("meta").unwrap().get("syns").unwrap().get(0).unwrap().as_array().unwrap();
        let word_type = entry.get("fl").unwrap().as_str().unwrap();

        let mut v: Vec<char> = entry.get("shortdef").unwrap().get(0).unwrap().as_str().unwrap().chars().collect();
